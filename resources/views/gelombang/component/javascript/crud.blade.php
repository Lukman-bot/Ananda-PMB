<!-- Script For CRUD Data Gelombang -->
<script>
    $(document).ready(function() {
        $("#form-gelombang").on('submit', (e) => {
            e.preventDefault();
            $("#btn-save").attr("disabled", "").html("Sedang upload")
            
            const nama_gelombang = $('[name="nama_gelombang"]').val()

            if (nama_gelombang) {
                const formData = new FormData($('#form-gelombang')[0]);
    ...
    const edit = async (id) => {
        $("#tambahLabel").html("Edit Data Gelombang")
        $.ajax({
            url: `{{url("gelombang/reqdata/")}}/${id}`,
            type: 'GET',
            success: (data) => {
                const res = JSON.parse(data)
                $("#form-modal").modal('show')
                $("[name='id']").val(id)
                $('[name="nama_gelombang"]').val(res.nama_gelombang)
...
<!-- End Script For CRUD Data Gelombang -->
